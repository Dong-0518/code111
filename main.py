...
def evaluate_classification_and_plot(model, data_loader, config, species_names):
    ...
    label_ids = np.arange(getattr(config, "NUM_CLASSES", len(species_names)))
    target_names = [species_names[int(i)] if int(i) < len(species_names) else str(i) for i in label_ids]

    # 先做计数矩阵，再手动按行归一化（对验证集中不存在的类别做安全处理）
    cm_count = confusion_matrix(y_true, y_pred, labels=label_ids)
    row_sum = cm_count.sum(axis=1, keepdims=True)
    cm = np.divide(cm_count, np.maximum(row_sum, 1), dtype=np.float64)
    missing_in_val = np.where(row_sum.squeeze() == 0)[0].tolist()

    # 保存分类汇总文本，便于排查“为什么准确率低”
    summary_path = os.path.join(config.OUTPUT_DIR, "figures", "validation_classification_summary.txt")
    with open(summary_path, "w", encoding="utf-8") as f:
        total_acc = float((y_true == y_pred).mean())
        f.write(f"Validation Accuracy: {total_acc:.4f}\n")
        f.write(f"Num classes: {len(label_ids)}\n")
        f.write(f"Samples: {len(y_true)}\n\n")
        if missing_in_val:
            f.write("Classes missing in validation set (row is all zero in confusion matrix):\n")
            for cid in missing_in_val:
                cname = target_names[cid]
                f.write(f"  - {cid}: {cname}\n")
            f.write("\n")
        f.write("Classification report (macro/weighted etc.):\n")
        f.write(classification_report(y_true, y_pred, labels=label_ids, target_names=target_names, zero_division=0))
